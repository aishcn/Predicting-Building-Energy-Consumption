...
### 3.3.3 Building charactersitics vs site eui
...
We also dealt with imbalanced categories, generating plots to study the relative frequency distribution of the categorical features. As observed in the EDA we saw imbalanced state factors, particularly with state 6, with the desparity suggesting we drop this altogether. We also note disparity between facility types, so we dealt with this by binning facility types into "Multifamily (uncategorized)" and "other" categories
...
As part of the preprocessing we encoded categorical features using ordinal encoding, where each unique category was assigned an integer value, resulting in a column of integers; 0 to n_categories -1 (per features)
...
#### 4.2.2. Pairwise Correlation Analysis
...
\
\
Table 3 shows all the transformations performed in order to achieve the desired results which are highlighted by Fig. 7 and Fig. 8.
...
For a simplified understanding (there is extensive theory on this method), we have have a dataset with multiple explanatory variables, where the x and y-axes, for example, can be thought of as two "components" (or dimensions) either of which the variables can be strongly correlated with. PCA is designed for continuous variables, and it tries to minimize the variance for each component. Instead of only two components, we can have many more.
...
For hyperparameter tuning, the reduced and transformed datasets were used. The hyperparameters were initialized and tuned as shown by the code, where Elastic Net, XGBoost, SVR, Gradient Boost and LGMB regressor were the methods used.
...
>>>>>>> a237f56f9f5389be5cefd1689055a8e64f1ba701
