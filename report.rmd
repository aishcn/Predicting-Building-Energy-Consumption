...
In this analysis, we sought to develop a model that predicts a site's energy consumption by studying the effects of climate variables and building characteristics on the Site EUI (Energy Usage Intensity) to provide crucial information for potential optimized energy retrofitting. This helps in improving a building's asset performance (utilities). Retrofitting offers a potential upside in the overall performance of the building through improved energy efficiency, increased staff productivity, reduced maintenance costs, and better thermal comfort. By training an ensemble of prediction models like XGBoost, Gradient Boosting Machine, Light GBM, and Support Vector Machine with important variables selected via variable importance method using a Random Forest Regressor and Correlation Analysis, we aim to make accurate Site EUI predictions. We achieved a high prediction performance, with the lowest Root Mean Square Error (RMSE) of 18.50 along with a 50% Interval Score of 44.03, Average Interval Length of 17.29 and a coverage rate of 0.493 for the Light GBM method.
...
*Due to the large number climate variables, summary statistics hasn't been provided in the report. Please refer to the code.*

### 3.2 Exploratory Data Analysis

```{r echo=FALSE, out.width='70%', fig.align="center"}
#knitr::include_graphics('Main_Code/images/all_climate_var_scatterplots.png')
```
...
\begin{center}
\textbf{Fig. 2}: Relationship between Building Variables and Site_EUI
\end{center}
...
```{r echo=FALSE, out.width='70%', fig.align="center"}
#knitr::include_graphics('Main_Code/images/all_building_var_scatterplots.png')
```
...
In a preliminary attempt of gathering information on covariate relationships with Site EUI, the researchers used Spearman rank correlation coefficients to investigate potential linear relationships with the response. Specifically, the researchers chose Spearman correlation, as it is invariant to the monotone increasing effect of transformations. We find that energy star rating has by far the largest Spearman correlation with Site EUI in terms of magnitude (-0.66), suggesting a relatively strong decreasing relationship with the response. We also note the relatively higher magnitudes for monthly min and average temperatures, particularly a decreasing relationship with colder months (namely January and February), and an increasing relationship with summer months such as June and July. Intuitively this could be explained; during colder and darker months there is a requirement for heating and lighting, which is not the case for summer months (less energy required).

### 3.3.2 Plots Investigating relationships with Site EUI

In order for the researchers to explore the data more effectively there are measures that can be taken such as subsetting the data, given the large size of the data set.
...
### 4.2.6 Dimensionality Reduction: Principal Component Analysis
...
<!-- Below is Anjalis summary from the presentation notes.

<!-- Anjali -->

<!-- Then since after feature selection, we still had a high dimensional dataset and to further deal with multicollinearity, we decided to perform PCA to achieve 95% of the variance in the data. And that gave us 6 Principal components. The figure on the right shows a 4d plot. This is not entirely representative of the Principal component space since we have 6 principal components but you can get an idea from this. PCA is based on svd (singular value decomposition) and linear and matrix algebra with eigenvalues and eigenvectors. I will not be covering the mathematical basis of PCA but to give you an idea, the plot basically shows clusters of samples based on their similarity. PCA does not discard any samples or characteristics (variables). Instead, it reduces the overwhelming number of dimensions by constructing principal components (PCs). PCs describe variation and account for the varied influences of the original characteristics. Such influences, or loadings, can be traced back from the PCA plot to find out what produces the differences among clusters. -->

<!-- Anjali -->

<!-- The first plot here shows that majority of the variance explained in our data is being captured by the principal component 1. And to each how each feature is playing a role in that..we look at the second plot. You have variable name on the y axis and relative importance on the x axis. The importance of each feature here is reflected by the magnitude of the corresponding values in the eigenvectors. The larger they are these absolute values, the more a specific feature contributes to that principal component. For the first PC, the most important features are seen with the ones having the highest absolute magnitude from the plot. -->


### 4.2.5 Hyperparameter Tuning

For hyperparameter tuning, the reduced and transformed datasets were used.
We used random search method with 10-fold repeated cross validation where we defined a search space or parameter space as a bounded domain of hyperparameter values and the random search randomly sample points in that domain to give an optimized value that performs the best out of all combinations. 
For example for ElasticNet, we are tuning it based on 24000 combinations of parameters. We have the definition provided here for all the hyperparameters we used, if you need more information on this, you can check the sklearn documentation on each of these models or each out to me and i can direct you to the right resources.
...
\begin{center}
\textbf{Fig. 17}: Comparing Insample Model Perfomance Metrics for Different Models
\end{center}
...
``` {r, echo=FALSE ,out.width="100%", out.height="90%",fig.cap="Building Characteristics Histograms",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="70%", out.height="65%",fig.cap="Floor Area Before and After Transformt",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="70%", out.height="70%",fig.cap="Floor_areaxElevation Before and After Transform",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="75%", out.height="75%",fig.cap="Freezing_days Before (no transform needed)t",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="70%", out.height="70%",fig.cap="Cold_days Before and After Transform",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="75%", out.height="75%",fig.cap="Warm_days Before and After Transform",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="75%", out.height="75%",fig.cap="Hot_days Before (no transform needed)",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE ,out.width="75%", out.height="75%",fig.cap="Principal Component Analysis 1/2",fig.show='hold',fig.align='center'}

knitr::include_graphics('Main_Code/images/pca_threshold.png')
knitr::include_graphics('Main_Code/images/pca_1st.png')
...
```{r, echo=FALSE ,out.width="75%", out.height="75%",fig.cap="Principal Component Analysis 2/2",fig.show='hold',fig.align='center'}

knitr::include_graphics('Main_Code/images/pca_curve.png')
...
knitr::include_graphics('Main_Code/images/rsq_boxplot.png')
knitr::include_graphics('Main_Code/images/pred_vs_act.png')
